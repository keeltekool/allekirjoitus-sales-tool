...
This document serves as the **content source** for all template types. Pricing is maintained separately in Part 6 and rendered as an independent, attachable pricing block — never inline within content sections.

### Template mapping

| Template | Parts used | Pricing block | Depth toggle |
|----------|-----------|---------------|-------------|
| **Enterprise Offer** | Parts 1-5, optionally 7-9 | Optional attachment (editable, pre-filled from Part 6) | Overview (description paragraphs only) / Deep-dive (full tables + sub-sections) |
| **Feature Introduction** | Parts 2-5, optionally 8 | Optional attachment | Overview / Deep-dive |
| **Pricing Proposal** | Part 1 (as "included" summary), selected items from 2-5, Part 7 | Hero element (editable amounts, pre-filled from Part 6) | Single depth |
| **Price List** | None (content-free) | The entire document — full Part 6, all line items, editable | Single depth |

### Section toggle logic

Content is toggled at the **layer level** (Parts 1-5, 8, 9), not at individual section level. Each layer renders as a complete, coherent block. Users remove individual sections they don't need directly from the preview.

### Status indicators

Each section carries a status label communicating what's included vs. what requires setup or additional cost:
- `[INCLUDED]` - part of the Enterprise subscription, no additional feature cost
- `[CONFIGURABLE]` - available, requires setup work
- `[ADD-ON]` - additional capability with its own pricing (see pricing block)
- `[INCLUDED / CONFIGURABLE]` - base capability included, advanced configuration available

Pricing details (EUR amounts, per-unit costs) live exclusively in Part 6 and the pricing block — never in content sections.
...
| Component | Your default | Can be extended to | Status |
|-----------|-------------|-------------------|--------|
| **Signature level** | Advanced Electronic Signature (AES) - eIDAS-compliant, legally binding across the EU | Qualified Electronic Signature (QES) via national ID cards and Smart-ID. Simple Electronic Signature (SES) via SMS OTP, Email OTP, drawn signature. | QES: Included / SES: Add-on |
...
| **Authentication - Nordic-Baltic** | Not active by default | Norwegian BankID, Vipps, Danish MitID, Smart-ID and Mobile-ID (Estonia, Latvia, Lithuania), Estonian e-Residency | Add-on |
| **Invitation delivery** | Email | SMS delivery or Email + SMS combined | SMS: Add-on (per message) |
| **Document retrieval** | OTP-protected download link via SMS | Automated retrieval and archiving via API/webhooks | Included (API) or Configuration work (custom archive connector) |
...
| **Document format** | PDF/A required (max 100 MB) | Office files and images accepted with auto-conversion | Add-on (per document) |
...
| **Portal branding** | Your organization's branding (logo, colors, fonts, background) on both portal and signer-facing pages | Per-department branding variations | One-time setup |
...
| **Portal URL** | companyname.allekirjoitus.fi | Fully custom domain (e.g., signing.yourcompany.fi) with dedicated SSL | Configuration work |
...
| Feature | Default state | Can be changed to | Status |
|---------|--------------|-------------------|--------|
...
| **Automatic reminders** | Active: 2 days after dispatch + 1 day before expiry | Custom schedules (specific intervals, SMS reminders) | Configuration work |
...
| **Archiving** | Manual download from portal | Automatic push to external archive (Zefort, Arkinet, custom) via API/webhooks | Configuration work |
...
| Method | Markets | How it works | Status |
|--------|---------|-------------|--------|
...
| Method | How the signer experiences it | Typical use | Status |
|--------|------------------------------|-------------|--------|
| **SMS OTP** | Receives a code on their phone, enters it in the signing portal | International signatories without Nordic/Baltic eID, internal approvals, supplier confirmations | Add-on (per signatory + per SMS) |
| **Email OTP** | Receives a code in their email inbox | When phone number is unavailable | Add-on (per signatory) |
| **InkSign** | Draws signature on touchscreen/tablet in a face-to-face setting | Bank branches, insurance offices, service counters - your staff verifies physical ID on site | Add-on (per signatory) |
...
| Aspect | Default | Configurable to | Status |
|--------|---------|----------------|--------|
| **Invitation delivery** | Email with secure signing link | SMS delivery, or email + SMS combined per recipient | SMS: Add-on (per message) |
| **Notification language** | Finnish | Swedish or English, selectable per request | Included |
| **Automatic reminders** | 2 days after dispatch + 1 day before expiry | Custom schedule with configurable intervals | Configuration work |
...
| **Automatic reminders** | 2 days after dispatch + 1 day before expiry | Custom intervals, SMS reminders |
...
| What your customers and employees see | Before setup | After setup | Status |
|--------------------------------------|-------------|-------------|--------|
| **Portal (your employees)** | Allekirjoitus.fi default theme | Your logo, colors, fonts, background | One-time setup |
| **Signing pages (your customers/signers)** | Allekirjoitus.fi default | Your branding - signers may not realize a third-party platform is involved | Included in branding setup |
| **Signed PDF cover page** | Allekirjoitus.fi logo | Your company logo + structured signatory metadata columns | Included in branding setup |
...
| Option | URL format | What the signer sees | Status |
|--------|-----------|---------------------|--------|
...
| What you control | Detail | Status |
|-----------------|--------|--------|
...
| **Message content** | Full control: email subject, body, HTML layout, footer, legal disclaimers | Included in branding setup |
...
| Service | Minimum | What it covers | Status |
|---------|---------|---------------|--------|
| **Development & Configuration** | 0.5 hours | Custom integrations, form development, template setup, tenant configuration, support/change requests | Hourly rate — see pricing block |
| **Specialist Consultation** | 0.5 hours | Architecture consultation, project management, solution design | Hourly rate — see pricing block |
| **Customization & Branding** | One-time | Portal UI, signer view, message templates, PAdES cover page | One-time fee — see pricing block |
...
*Master KB v2.0 - 2026-04-25*
*Architecture change: Pricing separated from content sections. EUR amounts removed from Parts 1-5. Part 6 is the sole pricing authority for the pricing block and price list template.*
